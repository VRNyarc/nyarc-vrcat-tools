    ...
    if (props.shapekey_source_object and actual_target and
        ...
        if props.shapekey_use_robust_transfer:
            ...
            if props.shapekey_show_advanced:
                ...
                advanced_col.separator(factor=2.0)

                # Post-Transfer Smoothing Section (collapsible)
                smoothing_box = advanced_col.box()
                smoothing_header = smoothing_box.row()
                smoothing_header.prop(props, "shapekey_show_post_smoothing",
                                     icon='TRIA_DOWN' if props.shapekey_show_post_smoothing else 'TRIA_RIGHT',
                                     icon_only=True, emboss=False)
                smoothing_header.label(text="Post-Transfer Smoothing", icon='MOD_SMOOTH')

                if props.shapekey_show_post_smoothing:
                ...
                if props.shapekey_show_island_handling:
                    ...
                    threshold_info.scale_y = 0.7
                    threshold_info.label(text="Max % of mesh to qualify as small island", icon='INFO')
                    threshold_info.label(text="(0.05 = 5% of total vertices)", icon='BLANK1')
